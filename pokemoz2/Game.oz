...
define
   ...
   Args = {Application.getArgs record(mapFile(single type:string default:'map.txt') probability(single type:float default:0.35) speed(single type:int default:4) autofight(single type:atom default:fight) auto(single type:bool default:true))}
   FightAuto = Args.autofight
   MapFile = Args.mapFile % have to be the name of the file
   {Float.toInt Args.probability*100.0 Probcast}
   Proba = Probcast mod 101 % must be less than 100
